...
#transform incident month into ordinal variable
obl_data$IncidentMonth <- ordered(obl_data$IncidentMonth, levels = 1:12,
                                  labels = c("January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"))

#not enough values per state, so create a variable that is FEMA Regions
obl_data$FEMARegion <- ifelse((obl_data$state == "CT" |
                                 obl_data$state == "ME" |
                                 obl_data$state == "MA" |
                                 obl_data$state == "NH" |
                                 obl_data$state == "RI" |
                                 obl_data$state == "VT"),
                              "Region I",
                              ifelse((obl_data$state == "NJ" |
                                         obl_data$state == "NY" |
                                         obl_data$state == "PR" |
                                         obl_data$state == "VI"),
                                      "Region II",
                                       ifelse((obl_data$state == "DE" |
                                                 obl_data$state == "MD" |
                                                 obl_data$state == "PA" |
                                                 obl_data$state == "VA" |
                                                 obl_data$state == "DC" |
                                              ...
                                              "Region III",
                                               ifelse((obl_data$state == "AL" |
                                                      ...
                                                      ifelse((obl_data$state == "IL" |
                                                               ...
                                                               obl_data$state == "WI"),
                                                            "Region V",
                                                             ifelse((obl_data$state == "AR" |
                                                                         obl_data$state == "LA" |
                                                                         obl_data$state == "NM" |
                                                                         obl_data$state == "OK" |
                                                                         obl_data$state == "TX"),
                                                                      "Region VI",
                                                                       ifelse((obl_data$state == "IA" |
                                                                                 obl_data$state == "KS" |
                                                                                 obl_data$state == "MO" |
                                                                                 obl_data$state == "NE"),
                                                                              "Region VII",
                                                                               ifelse((obl_data$state == "CO" |
                                                                                         obl_data$state == "MT" |
                                                                                         obl_data$state == "ND" |
                                                                                         obl_data$state == "SD" |
                                                                                         obl_data$state == "UT" |
                                                                                         obl_data$state == "WY"),
                                                                                      "Region VIII",
                                                                                       ifelse((obl_data$state == "AZ" |
                                                                                                 obl_data$state == "CA" |
                                                                                                 obl_data$state == "HI" |
                                                                                                 obl_data$state == "NV"),
                                                                                              "Region IX",
                                                                                               ifelse((obl_data$state == "AK" |
                                                                                                         obl_data$state == "ID" |
                                                                                                         obl_data$state == "OR" |
                                                                                                         obl_data$state == "WA"),
                                                                                                      "Region X",
...
obl_data$incidentType[(obl_data$incidentType == "Chemical" |
                        obl_data$incidentType == "Coastal Storm" |
                        obl_data$incidentType == "Dam/Levee Break" |
                        obl_data$incidentType == "Mud/Landslide" |
                        obl_data$incidentType == "Volcano" |
                        obl_data$incidentType == "Terrorist" |
                        obl_data$incidentType == "Snow")] <- "Other"

#remove Hurricane Maria outlier
obl_data <-subset(obl_data, OblAmt< 4000000000)
...
obl_folds <- rsample::vfold_cv(obl_train_data,
                               strata = incidentType,
                               v = 5,
                               repeats = 5)

#create recipe that codes categorical variables as dummy variables
OA_rec <- recipes::recipe(OblAmt ~ ., data = obl_train_data) %>%
...
obl_null_RMSE_train <- tibble::tibble(
                          rmse = rmse_vec(truth = obl_train_data$OblAmt,
                                              estimate = rep(mean(obl_train_data$OblAmt), nrow(obl_train_data))),
                          SE = 0,
                          model = "Null - Train")
...
obl_null_RMSE_test <- tibble::tibble(
                          rmse = rmse_vec(truth = obl_test_data$OblAmt,
                                              estimate = rep(mean(obl_test_data$OblAmt), nrow(obl_test_data))),
                          SE = 0,
                          model = "Null - Test")
```
...
tree_res <- tree_wflow %>%
                tune::tune_grid(resamples = obl_folds,
                                grid = tree_grid,
...
#save file
DT_auto_fig_file = here("results","DT-auto-plot-obl.png")
...
#save file
DT_detail_fig_file = here("results","DT-det-plot-obl.png")
ggsave(filename = DT_detail_fig_file, plot = DT_detail_plot)
...
DT_pred_act <- ggplot2::ggplot(tree_resid, aes(x = .pred, y = OblAmt)) +
                        geom_abline(slope = 1, intercept = 0, color = "red", lty = 2) + 
                        geom_point() +
                        labs(title = "Decision Tree Fit: Predicted vs. Actual Obligated Funds",
                              x = "Predicted Obligated Funds ($)",
                              y = "Actual Obligated Funds ($)")
DT_pred_act

#save file
DT_pred_act_file = here("results","DT-pred-act-obl.png")
...
#save file
DT_resid_file = here("results","DT-resid-obl.png")
...
DT_pred_resid <- ggplot2::ggplot(tree_resid, aes(x = .pred, y = .resid))+
                          geom_hline(yintercept = 0, color = "red", lty = 2) +
                          geom_point() +
                          labs(title = "Decision Tree Fit: Residuals vs Fitted Obligated Funds",
                                x = "Predicted Obligated Funds ($)",
                                y = "Residual")
DT_pred_resid

#save file
DT_pred_resid_file = here("results","DT-pred-resid-obl.png")
...
#save file
RF_auto_fig_file = here("results","RF-auto-plot-obl.png")
...
RF_pred_act <- ggplot2::ggplot(RF_resid, aes(x = .pred, y = OblAmt)) +
                        geom_abline(slope = 1, intercept = 0, color = "red", lty = 2) +
                        geom_point() +
                        labs(title = "RF Fit: Actual vs. Predicted Obligated Funds",
                              x = "Predicted Obligated Funds ($)",
                              y = "Actual Obligated Funds ($)")

#save file
RF_pred_act_file = here("results","RF-pred-act-obl.png")
...
#save file
RF_resid_file = here("results","RF-resid-obl.png")
...
RF_pred_resid <- ggplot2::ggplot(RF_resid, aes(x = .pred, y = .resid))+
                          geom_hline(yintercept = 0, color = "red", lty = 2) +
                          geom_point() +
                          labs(title = "Random Forest Fit: Residuals vs Fitted Obligated Funds",
                                x = "Fitted Requested Funds ($)",
                                y = "Residual")
RF_pred_resid

#save file
RF_pred_resid_file = here("results","RF-pred-resid-obl.png")
...
obl_data_bootsplit <- obl_train_data %>%
...
#create recipe that codes categorical variables as dummy variables
BT_rec <- recipes::recipe(OblAmt ~ ., data = obl_train_data) %>%
...
#tune the model with previously specified cross-validation and RMSE as target metric
BT_res <- BT_wflow %>%
          tune::tune_grid(resamples = obl_data_bootsplit,
                          grid = tree_grid,
                          metrics = metric_set(rmse),
...
#save file
BT_auto_fig_file = here("results","BT-auto-plot-obl.png")
...
#save file
BT_det_fig_file = here("results","BT-det-plot-obl.png")
...
#bag roots function to extract fit
...
#save file
BT_IV_file = here("results","BT-IV-obl-outlier.png")
...
#plot model predictions from tuned model versus actual outcomes
#geom_abline draws a 45 degree line, along which the results should fall
BT_pred_act <- ggplot2::ggplot(BT_resid, aes(x = .pred, y = OblAmt)) +
                        geom_abline(slope = 1, intercept = 0, color = "red", lty = 2) + 
                        geom_point() +
                        labs(title = "Bagged Tree Fit: Predicted vs. Actual Obligated Funds",
                              x = "Predicted Obligated Funds ($)",
                              y = "Actual Obligated Funds ($)")
BT_pred_act

#save file
BT_pred_act_file = here("results","BT-pred-act-obl.png")
...
#save file
BT_resid_file = here("results","BT-resids-obl.png")
...
BT_pred_resid <- ggplot2::ggplot(BT_resid, aes(x = .pred, y = .resid))+
                          geom_hline(yintercept = 0, color = "red", lty = 2) +
                          geom_point() +
                          labs(title = "Bagged Tree Fit: Residuals vs Fitted Obligated Funds",
                                x = "Predicted Obligated Funds ($)",
                                y = "Residual")
BT_pred_resid

#save file
BT_pred_resid_file = here("results","BT-pred-resid-obl.png")
...
#save combined table
model_comp_file = here("results", "ML-model-comp-obl.Rds")
...
#plot model predictions from tuned model versus actual outcomes
#geom_abline draws a 45 degree line, along which the results should fall
BT_pred_act_test <- ggplot2::ggplot(BT_resid_fit, aes(x = .pred, y = OblAmt)) +
                        geom_abline(slope = 1, intercept = 0, color = "red", lty = 2) + 
                        geom_point() +
                        labs(title = "Bagged Tree Test Fit: Predicted vs. Actual Obligated Funds",
                              x = "Predicted Obligated Funds ($)",
                              y = "Actual Obligated Funds ($)")
BT_pred_act_test

#save file
BT_pred_act_test_file = here("results","BT-pred-act-test-obl.png")
...
#save file
BT_resid_test_file = here("results","BT-resid-test-obl.png")
...
BT_pred_resid_test <- ggplot2::ggplot(BT_resid_fit, aes(x = .pred, y = .resid))+
                          geom_hline(yintercept = 0, color = "red", lty = 2) +
                          geom_point() +
                          labs(title = "Bagged Tree Test Fit: Residuals vs Fitted Obligated Funds",
                                x = "Predicted Obligated Funds ($)",
                                y = "Residual")
BT_pred_resid_test

#save file
BT_pred_resid_test_file = here("results","BT-pred-resid-test-obl.png")
